...
const CourseCard = ({ course, courseData, onStartCourse, trackIcon }) => {
    const getStatusStyles = () => {
        ...
        return {
            borderColorClass: statusClass.split(' ')[0],
            textColorClass: statusClass.split(' ').slice(1).join(' '),
            text: statusInfo.text,
            subText: courseData?.dueDate && statusInfo.color !== 'green'
                ? `Due: ${new Date(courseData.dueDate + "T23:59:59Z").toLocaleDateString()}`
                : ''
        };
    };
    ...
    return ( 
        <div 
            ...
            className={`bg-white dark:bg-neutral-800 rounded-lg shadow-md dark:shadow-neutral-900 p-4 border-l-4 ${styles.borderColorClass} cursor-pointer hover:shadow-lg dark:hover:bg-neutral-700 transition-all`}
        > 
            ...
            <p className={`text-sm font-semibold ${styles.textColorClass} mt-2`}>{styles.text}</p> 
